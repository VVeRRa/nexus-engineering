  ...
  return (
    ...
    >
      <div className="sticky top-0 h-[100dvh] w-full overflow-hidden flex flex-col justify-center items-center">
        <Background3D key={locale} scrollYProgress={scrollYProgress} />
        ...
        >
          ...
          <div className="relative w-full text-center">
            <h1 className="text-3xl sm:text-5xl md:text-[128px] font-bold text-white tracking-[-0.05em] leading-none uppercase inline-block py-8">
...
function TextChunk({ text, progress, range, isGradient }: { text: string; progress: any; range: [number, number, number, number]; isGradient?: boolean }) {
  ...
  return (
    ...
    >
      <h2 className={`text-3xl sm:text-5xl md:text-[128px] font-bold tracking-[-0.05em] uppercase selection:bg-white selection:text-black leading-none py-12 ${isGradient ? 'bg-gradient-to-r from-blue-400 to-purple-600 bg-clip-text text-transparent' : 'text-white'}`}>
        {text}
      </h2>
    </motion.div>
  );
