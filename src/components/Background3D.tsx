...
function TheCore({ scrollYProgress }: { scrollYProgress: any }) {
    ...
    return (
        <group>
            {/* Outer Globe - Blue (Low Poly) */}
            <mesh ref={mesh}>
                <sphereGeometry args={[1.5, 16, 16]} />
                <meshBasicMaterial color="#0070f3" wireframe transparent opacity={0.2} blending={THREE.AdditiveBlending} depthWrite={false} />
            </mesh>
            {/* Inner Globe - Cyan (Low Poly) */}
            <mesh ref={innerMesh}>
...
function GalaxySwarm({ scrollYProgress }: { scrollYProgress: any }) {
    ...
    const [sphere] = useState(() => {
        // Ultra-low particle count for stability
        const count = 100;
